...
/**
 * Description of PreprodMailFormDataUpdate
 * Migrate "mailform" data to new 8.0 ext:form
 *  
 * @author sebastienhupin
 */
class PreprodMailFormDataUpdate extends \TYPO3\CMS\Install\Updates\AbstractUpdate {
    /**
     *
     * @var string
     */
     ...
     * @var string
     */
    protected $title = 'Opentalent : Preprod correcting the wrong path in flexform Migrate "mailform" data to new 7.6 ext:form';
    /**
    ...
    {
        $customMessage = '<br /><br />Start Migrate mail form';
        $query = $this->getMigratesQuery();
        ...
        while ($row = $statement->fetch()) {
            ...
            $customMessages .= sprintf('<br /><br />Opentalent migrate tt_content %d for root page %d and structure id %d ', $this->currentRow['uid'], $rootPageUid, $structureId);
...
//                die();
    ...
    {        
        ...
        ->where(
            $queryBuilder->expr()->eq('tt_content.CType', $queryBuilder->createNamedParameter('form_formframework'))        
        )                        
        ;        
