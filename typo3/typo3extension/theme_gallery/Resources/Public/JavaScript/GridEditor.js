/**
 * Module: TYPO3\CMS\ThemeGallery\GridEditor
 */
define(['jquery', 'TYPO3/CMS/Backend/Modal', 'TYPO3/CMS/Backend/Severity', 'TYPO3/CMS/Lang/Lang', 'bootstrap'], function($, Modal, Severity, Lang) {
	 ...
	 * @exports TYPO3\CMS\ThemeGallery\GridEditor
	 */
	var GridEditor = {
		...
		targetElement: null,
		defaultCell: {spanned: 0, rowspan: 1, colspan: 1, name: '', colpos: '', allowed: '', allowedGridTypes: '', maxElement: 1000}
	};
					...
					$('<div class="cell_data">')
						.html(
							...
							+ (cell.allowedGridTypes ? GridEditor.stripMarkup(cell.allowedGridTypes) : '*')
                                                        + '<br />'
                                                        + Lang['grid_maxElement'] + ': '
                                                        + (cell.maxElement ? parseInt(cell.maxElement, 10) : 1000)
	...
	/**
	 * Sets the max element for column.
	 *
	 * @param {String} newMaxElement
	 * @param {Integer} col
	 * @param {Integer} row
	 *
	 * @returns {Boolean}
	 */
	GridEditor.setMaxElement = function(newMaxElement, col, row) {
		var cell = GridEditor.getCell(col, row);
		if (!cell) {
			return false;
		}
		cell.maxElement = newMaxElement.trim() !== '' ? parseInt(newMaxElement,10) : 1000;
		return true;
	};
	/**
		...
		$markup.append([
			...
			$formGroup
				...
				.append([
					$label
						.clone()
						.text(Lang['grid_maxElementHelp'])
					,
					$input
						.clone()
						.attr('type', 'text')
						.attr('class', 't3js-grideditor-field-max-element form-control')
						.attr('name', 'maxElement')
						.val(parseInt(cell.maxElement,10) || 1000)
			])                        
		]);
		...
		$modal.on('button.clicked', function(e) {
			if (e.target.name === 'cancel') {
				...
				GridEditor.setAllowedGridTypes($modal.find('.t3js-grideditor-field-allowed-grid-types').val(), $modal.data('col'), $modal.data('row'));
                                GridEditor.setMaxElement($modal.find('.t3js-grideditor-field-max-element').val(), $modal.data('col'), $modal.data('row'));
				GridEditor.drawTable();
				GridEditor.writeConfig(GridEditor.export2LayoutRecord());
				Modal.currentModal.trigger('modal-dismiss');
			}
				...
				if (cell && !cell.spanned) {
					...
					if (cell.allowedGridTypes !== undefined && cell.allowedGridTypes !== '' && cell.allowedGridTypes !== '*') {
						result += "\t\t\t\t\tallowedGridTypes = " + cell.allowedGridTypes + "\n";
                                        ...
                                        result += "\t\t\t\t\tmaxElement = " + (cell.maxElement ? cell.maxElement : 1000) + "\n";
					result += "\t\t\t\t}\n";
				}
