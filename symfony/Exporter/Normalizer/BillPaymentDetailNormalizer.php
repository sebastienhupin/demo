 ...
 */
class BillPaymentDetailNormalizer implements NormalizerInterface {

    use ReflectionTrait;

    const FORMAT = 'bill-payment-detail';
    ...
    public function normalize(ExportParameters $exportParameters, Array $data) {
        ...
        foreach($bill['billPayments'] as $billPayments){
            if($billPayments["isAcquired"]){
                $paidPayments[] = $billPayments;
            }
        }

        $bill['paidPayments'] = $paidPayments;
