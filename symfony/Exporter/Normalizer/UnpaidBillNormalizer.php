 ...
 */
class UnpaidBillNormalizer implements NormalizerInterface {

    use ReflectionTrait;

    const FORMAT = 'unpaid-bill';
    ...
    public function normalize(ExportParameters $exportParameters, Array $data) {
        ...
        foreach($data as $bill){
            if($bill["paymentState"] !== PaymentStateEnum::SOLDIER){
                $nonAcquiredPayments = [];
                $acquiredPayments = [];
                foreach($bill["billPayments"] as $billPayment){
                    if($billPayment["isAcquired"]){
                        ...
                        $nonAcquiredPayments[] = $billPayment;
                    }
                }
                $bill["billPayments"] = [
                    "nonAcquiredPayments" => $nonAcquiredPayments,
        ...
        if(count($bills) === 0){
            throw new \RuntimeException('no-bill-unpaid');
        }
        ...
        if (false === $domDocument) {
            throw new \RuntimeException('unpaid_bill_error');
        }
